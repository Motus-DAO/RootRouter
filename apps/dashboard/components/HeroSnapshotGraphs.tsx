...
function HeroSnapshotGraphsInner() {
...
export default function HeroSnapshotGraphs() {
  ...
  if (!convexAvailable) {
    return (
      ...
      >
        <div>
          <p style={{ marginBottom: '0.5rem' }}>Convex not configured.</p>
          <p style={{ margin: 0, opacity: 0.9 }}>
            Add <code style={{ padding: '0.1rem 0.3rem', background: 'rgba(0,0,0,0.3)', borderRadius: 4 }}>NEXT_PUBLIC_CONVEX_URL</code> in <code>apps/dashboard/.env.local</code> to see snapshots here.
          </p>
        </div>
      </div>
    );
  }
  return <HeroSnapshotGraphsInner />;
}
