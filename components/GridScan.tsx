...
function smoothDampVec2(
  ...
  const targetVal = current.clone().sub(change);
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime);
  currentVelocity.sub(temp.clone().multiplyScalar(omega));
  currentVelocity.multiplyScalar(exp);

  out.copy(targetVal.clone().add(change.add(temp).multiplyScalar(exp)));
...
export function GridScan({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      ...
      (materialRef.current.uniforms as Record<string, { value: number }>).uScanCount.value = arr.length;
    }
  };
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const uniforms = {
        iResolution: {
          value: new THREE.Vector3(w, h, renderer.getPixelRatio()),
        },
      iTime: { value: 0 },
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      ...
      (bloomRef.current as unknown as { luminanceMaterial: { threshold: number; smoothing: number } })
  ...
  return (
    <div
      ref={containerRef}
      // We don't rely on utility classes here because this project
      // isn't using Tailwind; enforce full-size via inline styles.
      className={className}
      style={{
        position: 'relative',
        width: '100%',
